...
def extract_regions(img, threshold_min=50, connect_tol=0.05):
    binary = (img >= threshold_min).astype(np.uint8)
    labeled = label(binary)
    props = regionprops(labeled, intensity_image=img)

    regions = []
    for region in tqdm(props, desc='Extracting regions'):
        ...
        regions.append((region.mean_intensity, mask))
...
if __name__ == '__main__':
    ...
    regions = extract_regions(proc, threshold_min=th_min, connect_tol=connect_tol)
